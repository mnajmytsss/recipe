...
const /** {NodeElement} */ $filterSearch = $filterBar.querySelector("#search");
...
queryStr &&
  queryStr.split("&").map((i) => {
    if (i.split("=")[0] === "q") {
      $filterBar.querySelector("#search").value = i
        .split("=")[1]
        .replace(/%20/g, " ");
    } else {
      const checkboxValue = i.split("=")[1].replace(/%20/g, " ");
      const checkbox = $filterBar.querySelector(`[value="${checkboxValue}"]`);
      if (checkbox) {
